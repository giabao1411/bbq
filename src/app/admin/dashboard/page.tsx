            ...
            <section className="lg:col-span-12 bg-[#121414] border border-[#2d2c30] rounded-2xl p-6">
              <div className="flex justify-between items-center mb-md">
                ...
                <div className="flex gap-2 bg-[#1c1b1f] p-1 rounded-lg text-xs border border-white/5">
                  <button onClick={() => setFilterType('all')} className={`px-3 py-1.5 rounded-md ${filter_type === 'all' ? 'bg-[#990011] text-white shadow-sm font-medium' : 'text-gray-400 font-medium'}`}>Tất cả</button>
                  <button onClick={() => setFilterType('this_week')} className={`px-3 py-1.5 rounded-md ${filter_type === 'this_week' ? 'bg-[#990011] text-white shadow-sm font-medium' : 'text-gray-400 font-medium'}`}>Tuần này</button>
                </div>
              </div>
